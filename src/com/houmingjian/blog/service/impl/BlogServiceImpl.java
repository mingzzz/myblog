...
public class BlogServiceImpl implements BlogServiceInter {
	...
	public Page getSearchPaper(String queryData, Page page) {
		BlogPaperQueryVo vo = new BlogPaperQueryVo();
		vo.setPage(page);
		vo.setTitle(queryData);
		
		page.setTotalRecNum(blogPaperMapperCustom.getSearchBlogPaperCount(queryData));
		System.out.println("st:"+vo.getPage().getStartIndex());
		System.out.println("ed:"+vo.getPage().getEndIndex());
		page.setPageContent(blogPaperMapperCustom.getLimitSearchBlogPaper(vo));
		return page;
	}
